...
usarlo justo antes de enviar una orden para aplicar límites de exposición.
"""
...
def fingerprint_orden(symbol: str, side: str, qty: float | str, order_type: str = "market") -> str:
    """Huella estable para logs/telemetría, no para reutilizar como ID."""
    payload = "|".join((normalizar_simbolo(symbol), str(side or "").lower(), str(order_type or "").lower(), str(qty)))
...
__all__ = [
    "normalizar_simbolo", "crear_client_order_id", "fingerprint_orden",
    "posicion_notional", "orden_buy_notional", "exposicion_actual",
    "compras_abiertas_notional", "validar_exposicion_compra", "backoff_seconds",
]
