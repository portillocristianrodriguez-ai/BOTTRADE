...
from ai_analyst_runtime import simulate_risk_change
...
def learn(trades: Sequence[Mapping[str, Any]], metrics: Mapping[str, Any], current_config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    Requiere muestras amplias para evitar sobreajuste. Las mejoras de riesgo se
    limitan a cambios pequeños y todas pasan por la allowlist del AI gate.
    """
    sample = list(trades)
    ...
    if prior and recent["expectancy"] is not None and prior["expectancy"] is not None:
        delta = recent["expectancy"] - prior["expectancy"]
        observations.append(f"Cambio de expectativa último bloque vs. bloque anterior: {delta:+.4f}.")

    proposal: Proposal | None = None
    current_risk = _num(current_config.get("RISK_PER_TRADE_PCT"), 0.02) or 0.02
    max_dd = _num(metrics.get("max_drawdown"), 0.0) or 0.0

    # Protección primero: deterioro real + drawdown -> bajar riesgo un solo escalón.
    if len(sample) >= 30 and (recent["expectancy"] or 0) < 0 and max_dd <= -0.04:
        target = round(current_risk * 0.80, 6)
        if 0 < target < current_risk:
            validation = simulate_risk_change(metrics, current_risk, target)
            proposal = make_proposal(
                ...
                validation={**validation, "learning_sample": len(sample), "recent_stats": recent},
            )

    # Solo se permite aumentar riesgo con edge estable y muestra grande; el salto es pequeño.
        ...
        if target > current_risk and target <= min(0.05, current_risk * 1.25):
            validation = simulate_risk_change(metrics, current_risk, target)
            proposal = make_proposal(
                "Aprendizaje adaptativo: edge positivo y estable con muestra suficiente.",
                {"RISK_PER_TRADE_PCT": target},
                expected_impact="Aumentar moderadamente el riesgo solo después de evidencia histórica robusta; no garantiza mayor rentabilidad futura.",
                validation={**validation, "learning_sample": len(sample), "recent_stats": recent, "guardrail": "max 5% risk/trade and +10% step"},
            )
    ...
    return {
        ...
        "policy": "rolling learning; one small proposal at a time; explicit approval required",
    }
