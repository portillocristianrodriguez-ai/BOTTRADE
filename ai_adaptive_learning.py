...
_STRATEGY_KEYS=(
    ...
    "ATR_STOP_MULTIPLICADOR","ATR_TAKE_PROFIT_MULTIPLICADOR",
    "EMA_RAPIDA","EMA_LENTA","EMA_TENDENCIA","RSI_PERIODO",
    "RSI_SOBRECOMPRA","RSI_SOBREVENTA","CRYPTO_SCORE_MINIMO",
    "CRYPTO_MIN_MOMENTUM_PCT","CRYPTO_VOLUME_MIN_MULTIPLICADOR",
    "CRYPTO_RSI_MIN","CRYPTO_RSI_MAX",
)
...
def _parameter_candidates(trades: Sequence[Mapping[str,Any]], current_config: Mapping[str,Any]):
    """Busca valores históricos alternativos con evidencia suficiente.

    Solo compara operaciones que llevan guardado el contexto de entrada. Esto
    evita que la IA atribuya causalidad a parámetros que nunca se utilizaron.
    """
    best=None
    for key in _STRATEGY_KEYS:
        ...
        for trade in trades:
            context=trade.get("entry_config") or {}
        ...
        current_stats=_stats(current_group)
        current_exp=_num(current_stats.get("expectancy"))
        current_pf=_num(current_stats.get("profit_factor"))
        if current_exp is None: continue
        for candidate,items in groups.items():
            if candidate==round(current,8) or len(items)<20: continue
            candidate_stats=_stats(items); candidate_exp=_num(candidate_stats.get("expectancy")); candidate_pf=_num(candidate_stats.get("profit_factor"))
            if candidate_exp is None or candidate_pf is None: continue
            improvement=candidate_exp-current_exp
            relative=improvement/max(abs(current_exp),0.01)
            if candidate_pf<1.20 or improvement<=0 or relative<0.15: continue
            score=(relative,candidate_pf,len(items))
            if best is None or score>best["score"]:
                best={"key":key,"current":current,"candidate":candidate,"current_stats":current_stats,"candidate_stats":candidate_stats,"score":score}
    return best
...
def learn(trades: Sequence[Mapping[str, Any]], metrics: Mapping[str, Any], current_config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if len(sample)>=30 and recent["expectancy"] is not None and overall["expectancy"] is not None:
        ...
        if drift<0:observations.append(f"La expectativa reciente está {abs(drift):.4f} por debajo de la media histórica.")
        elif drift>0:observations.append(f"La expectativa reciente mejora {drift:.4f} frente a la media histórica.")
    ...
    stock_risk=_num(current_config.get("RISK_PER_TRADE_PCT"),0.02) or 0.02; crypto_risk=_num(current_config.get("CRYPTO_RISK_PER_TRADE_PCT"),0.01) or 0.01; max_dd=_num(metrics.get("max_drawdown"),0.0) or 0.0
    dominant_type=None
    if by_type:
