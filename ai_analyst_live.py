...
def analyze_now(broker):
    ...
    trades=[dict(r.get("payload",{})) for r in trades_records]; equity_curve=[dict(r.get("payload",{})) for r in equity_records]; report=_runtime().analyze(trades,equity_curve,_positions(broker))
    proposal=propose_risk_reduction(report["metrics"],float(getattr(config,"RISK_PER_TRADE_PCT",0.02)))
    if proposal is None and report.get("learning",{}).get("proposal"):
        raw=report["learning"]["proposal"]
...
def format_report(report):
    ...
    for observation in learning.get("observations",[])[:2]:lines.append(f"🧠 APRENDIZAJE: {observation}")
...
def install(bot):
    global _STARTED
    if _STARTED:return
    _STARTED=True; original=getattr(bot,"procesar_comando_telegram",None)
