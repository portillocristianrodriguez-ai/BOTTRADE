...
<div class="section card"><div class="sectionhead"><h2>MOVIMIENTOS EJECUTADOS</h2><span class="muted">últimas ejecuciones reportadas por Alpaca</span></div><div class="notice">El importe de una compra es dinero invertido; el de una venta es dinero recibido, no beneficio. El beneficio realizado requiere el coste de adquisición de las unidades vendidas y las comisiones; esta ventana de 50 ejecuciones no permite verificarlo.</div><div id="fills" class="table"><div class="empty">Cargando…</div></div></div>
...
const money=n=>!valid(n)?'—':Number(n)===0?'$0.00':Math.abs(Number(n))<0.01?(Number(n)<0?'-':'')+'<$0.01':new Intl.NumberFormat('en-US',{style:'currency',currency:'USD',maximumFractionDigits:2}).format(Number(n));
const precise=n=>!valid(n)?'—':Number(n)===0?'$0.00':Math.abs(Number(n))<0.00000001?'$'+Number(n).toExponential(4):new Intl.NumberFormat('en-US',{style:'currency',currency:'USD',minimumFractionDigits:2,maximumFractionDigits:Math.abs(Number(n))<1?10:4}).format(Number(n));
const num=n=>!valid(n)?'—':Number(n)!==0&&Math.abs(Number(n))<1e-12?Number(n).toExponential(4):new Intl.NumberFormat('en-US',{maximumFractionDigits:12}).format(Number(n));
const pct=n=>!valid(n)?'—':Number(n)===0?'0.00%':Math.abs(Number(n))<0.005?(Number(n)<0?'-':'+')+'<0.01%':(Number(n)>0?'+':'')+Number(n).toFixed(2)+'%';
const esc=s=>String(s??'').replace(/[&<>\"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;',"'":'&#39;'}[c]));
const cls=n=>!valid(n)||Number(n)===0?'neutral':Number(n)>0?'good':'bad';
...
function positionResult(x){
 const q=valid(x.qty)?Number(x.qty):null, entry=valid(x.avg_entry_price)?Number(x.avg_entry_price):null, last=valid(x.current_price)?Number(x.current_price):null;
 const ok=q!==null&&q!==0&&entry!==null&&entry>0&&last!==null&&last>0;
 return {value:ok?Math.abs(q*last):null,pnl:ok?q*(last-entry):null,ret:ok?Math.sign(q)*(last/entry-1)*100:null,move:ok?q*last*0.01:null};
}
function renderPositions(items,preserveOrder=false){if(!items?.length)return '<div class="empty">Sin posiciones abiertas.</div>';return '<table><thead><tr><th>Activo</th><th>Cantidad</th><th>Entrada / unidad</th><th>Precio / unidad</th><th>Valor estimado</th><th>G/P pendiente</th><th>Retorno</th><th>Sube 1%</th><th>Baja 1%</th></tr></thead><tbody>'+(preserveOrder?[...items]:[...items].sort((a,b)=>(positionResult(b).value??0)-(positionResult(a).value??0))).map(x=>{const r=positionResult(x);return `<tr><td><b>${esc(x.symbol)}</b>${r.value!==null&&r.value<0.01?' <span class="pill">Residuo &lt; $0.01</span>':''}</td><td>${num(x.qty)}</td><td>${precise(x.avg_entry_price)}</td><td>${precise(x.current_price)}</td><td>${money(r.value)}</td><td class="${cls(r.pnl)}">${money(r.pnl)}</td><td class="${cls(r.ret)}">${pct(r.ret)}</td><td class="${cls(r.move)}">${money(r.move)}</td><td class="${cls(r.move===null?null:-r.move)}">${money(r.move===null?null:-r.move)}</td></tr>`}).join('')+'</tbody></table><div class="meta">±1%: cambio adicional hipotético del resultado desde el precio actual, con la misma cantidad; no es una previsión ni una operación realizada.</div>'}
function renderOrders(items){if(!items?.length)return '<div class="empty">Sin órdenes recientes.</div>';return '<table><thead><tr><th>Created</th><th>Symbol</th><th>Side</th><th>Type</th><th>Qty</th><th>Status</th><th>Filled qty</th><th>Avg fill</th></tr></thead><tbody>'+items.map(x=>`<tr><td>${esc(x.created_at)}</td><td><b>${esc(x.symbol)}</b></td><td>${esc(x.side)}</td><td>${esc(x.type)}</td><td>${esc(x.qty)}</td><td>${pill(x.status)}</td><td>${esc(x.filled_qty??'—')}</td><td>${x.filled_avg_price?precise(x.filled_avg_price):'—'}</td></tr>`).join('')+'</tbody></table>'}
function renderFills(items){if(!items?.length)return '<div class="empty">Sin ejecuciones disponibles.</div>';return '<table><thead><tr><th>Fecha</th><th>Activo</th><th>Movimiento</th><th>Cantidad ejecutada</th><th>Precio / unidad</th><th>Importe bruto</th><th>Beneficio realizado</th></tr></thead><tbody>'+items.map(x=>`<tr><td>${esc(x.date)}</td><td><b>${esc(x.symbol)}</b></td><td>${x.side==='buy'?'Compra':x.side==='sell'?'Venta':esc(x.side)}</td><td>${num(x.qty)}</td><td>${precise(x.price)}</td><td>${money(valid(x.qty)&&valid(x.price)?Number(x.qty)*Number(x.price):null)} ${x.side==='buy'?'invertidos':x.side==='sell'?'recibidos':''}</td><td class="neutral">No verificable con esta ventana</td></tr>`).join('')+'</tbody></table>'}
...
def state(period: str):
    ...
    clean_positions=[{k:p.get(k) for k in ("symbol","asset_class","qty","avg_entry_price","current_price","market_value","unrealized_pl","unrealized_plpc")} for p in positions]
