...
def obtener_datos(ticker: str) -> pd.DataFrame:
    ...
    try:
        ...
        if es_cripto(ticker):
            ...
            resultado = (
                cliente_datos_cripto
                .get_crypto_bars(request)
            )
        ...
        if not es_cripto(ticker):
            ...
            if len(df) < 50:
                ...
                log.warning(
                    f"{ticker}: solo se obtuvieron "
                    f"{len(df)} velas diarias."
                )
...
def proteger_posicion(
    ...
    """
    Crea protección OCO para una posición
...
def calcular_tamano_posicion(
    ...
    Cripto:
        Posición fraccionada con límites
        de notional y buying power.
    """
    ...
    try:
        ...
        if es_cripto(ticker):
            ...
            # Máximo 20% del capital por posición.
            max_por_posicion = (
                equity
                * 0.20
            )

            # Máximo permitido por orden.
            max_notional_alpaca = (
                200000.0
            )

            # Dejamos margen de buying power.
            max_buying_power = (
                buying_power
                * 0.90
            )
...
def perdida_pct_no_realizada(
    ...
    """
    Devuelve la ganancia/pérdida porcentual
    no realizada.
    """
...
def obtener_ordenes_ejecutadas():
    """
    Obtiene órdenes cerradas.

    Usa GetOrdersRequest porque la versión
...
def inicializar_monitor_ejecuciones():
    """
    Marca las órdenes existentes como conocidas.

    De esta forma no se envían notificaciones
    de operaciones antiguas al arrancar.
    """
