    ...
    if callable(original_error):
        def registrar_error_filtrado(origen, error):
            if str(origen or "").startswith("observacion_acciones_"):
                main_module.log.debug("[seguridad] Error de observación aislado: %s: %s", origen, error)
                return
...
def _resumen_crypto(main_module):
    try:
        ...
        estables = {"USDT", "USDC", "DAI", "USDG", "PYUSD", "USDS", "FDUSD", "TUSD", "USDP", "GUSD", "EURC"}
        universo = []
        for activo in activos:
            simbolo = str(getattr(activo, "symbol", "")).upper().strip().replace("-", "/")
            ...
            if clase and "crypto" not in clase:
                continue
            if main_module.config.CRYPTO_EXCLUIR_ESTABLES and simbolo.split("/")[0] in estables:
                continue
            ...
            for posicion in posiciones:
                ...
                try:
                    ...
                    if bool(analisis.get("tiene_proteccion", False)):
                        ...
                        continue
                    main_module.log.warning("[protección] %s: posición sin protección completa; intentando recuperarla.", ticker)
                    main_module.proteger_compra_ejecutada(ticker)
                except Exception as exc:
                    main_module.marcar_posicion_sin_proteccion(ticker)
                    main_module.log.error("[protección] %s: fallo en watchdog universal: %s", ticker, exc)
            if posiciones:
                main_module.log.info("[protección] Verificación universal: %s posiciones (%s acciones, %s crypto).", len(posiciones), acciones, criptos)
        except Exception as exc:
            main_module.log.warning("[protección] Watchdog universal no pudo consultar posiciones: %s", exc)
