...
def calcular_indicadores(df):
    ...
    # ------------------------------------------------------------
    # NUEVO: protección contra ratios de volumen absurdos.
    #
    # Cuando volumen_media es un número extremadamente pequeño
    # (pero no exactamente 0, por lo que replace(0, NA) no lo
    # captura), la división puede producir ratios sin sentido
    # como 6x10^11. Esto no rompe la lógica de compra (se compara
    # con >=), pero contamina pattern_observations.jsonl con
    # datos basura que degradan cualquier análisis futuro.
    #
    # Se exige un mínimo absoluto de volumen medio antes de
    # calcular el ratio, y se limita el resultado a un techo
    # razonable.
    # ------------------------------------------------------------
    VOLUMEN_MEDIA_MINIMO = 1e-8
    RATIO_VOLUMEN_MAXIMO = 100.0

    volumen_media_segura = df["volumen_media"].where(
        df["volumen_media"] > VOLUMEN_MEDIA_MINIMO
    )

    df["volumen_ratio"] = pd.to_numeric(
        df["volume"] / volumen_media_segura,
        errors="coerce",
    ).clip(upper=RATIO_VOLUMEN_MAXIMO)

    df["volumen_media_corta"] = df["volume"].shift(1).rolling(3, min_periods=3).mean()

    volumen_media_corta_segura = df["volumen_media_corta"].where(
        df["volumen_media_corta"] > VOLUMEN_MEDIA_MINIMO
    )

    df["aceleracion_volumen"] = pd.to_numeric(
        df["volume"] / volumen_media_corta_segura,
        errors="coerce",
    ).clip(upper=RATIO_VOLUMEN_MAXIMO)
