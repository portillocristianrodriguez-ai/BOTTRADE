...
def test_valores_no_finitos_se_descartan():
    out = volume_data_hardening._sanitize(
        _df([100] * 6, ratio=[1.0, 1.0, 1.0, 1.0, float("inf"), 2.0], accel=[1.0, 1.0, 1.0, 1.0, float("nan"), 2.0]),
        _config(5),
    )
...
def test_ratio_normal_se_conserva():
    out = volume_data_hardening._sanitize(
        _df([100] * 6, ratio=[1.0] * 6, accel=[1.5] * 6), _config(5)
    )
...
def test_volumen_no_finito_no_es_valido():
    out = volume_data_hardening._sanitize(
        _df([100, 100, 100, 100, np.inf, 100], ratio=[1.0] * 6, accel=[1.0] * 6), _config(5)
    )
